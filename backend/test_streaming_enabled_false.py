"""测试streaming_enabled字段是否能正确更新为false"""
import requests
import json
...
print("=== 测试streaming_enabled字段更新为false ===")
...
# 2. 只修改streaming_enabled字段为false
print("\n2. 只修改streaming_enabled字段为false...")
patch_data = {"streaming_enabled": False}
response = requests.patch(url, headers=headers, data=json.dumps(patch_data))
print(f"状态码: {response.status_code}")
print(f"响应内容: {json.dumps(response.json(), indent=2, ensure_ascii=False)}")

# 3. 再次获取系统设置，检查streaming_enabled字段是否被修改为false
print("\n3. 再次获取系统设置，检查streaming_enabled字段是否被修改为false...")
response = requests.get(url)
print(f"状态码: {response.status_code}")
...
# 5. 检查streaming_enabled字段是否被正确修改为false
print("\n5. 检查streaming_enabled字段是否被正确修改为false...")
if "streaming_enabled" in updated_settings and updated_settings["streaming_enabled"] == False:
    print("✅ streaming_enabled: 已成功修改为 False")
...
if streaming_updated and all_unchanged:
    print("\n🎉 测试通过! streaming_enabled字段已成功修改为False，其他字段保持不变")
else:
