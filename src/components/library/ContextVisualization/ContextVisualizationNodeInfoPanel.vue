<template>
  ...
  <div v-if="selectedNode" id="nodeInfoPanel" class="absolute top-10 right-4 h-[calc(100%-60px)] w-[16rem] bg-dark/80 backdrop-blur-md z-40 border border-blue-400/30 flex flex-col transform transition-transform duration-300 rounded-[12px] overflow-hidden">
    <div class="p-3 flex justify-between items-center border-b border-blue-400/30">
      <h2 class="text-lg font-bold text-blue-300">节点信息</h2>
      <button
    ...
    <div class="overflow-y-auto flex-1 p-4 space-y-6">
      <!-- 节点名称和描述 -->
      <div class="mb-6">
        <h3 class="text-xl font-bold text-blue-300 mb-2">{{ selectedNode.name }}</h3>
        <p class="text-blue-200 text-sm">{{ selectedNode.description }}</p>
      </div>
      
      <!-- 节点属性 -->
      <div class="mb-6 border-b border-gray-700/50 pb-4">
        <h4 class="text-blue-300 font-semibold mb-2">节点属性</h4>
        <div class="space-y-2 text-sm">
          <div class="flex justify-between">
            <span class="text-blue-300">ID:</span>
            <span class="text-blue-200">{{ selectedNode.id }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-blue-300">组:</span>
          ...
          <div class="flex justify-between">
            <span class="text-blue-300">大小:</span>
            <span class="text-blue-200">{{ selectedNode.size }}</span>
          </div>
        </div>
      </div>
      ...
      <!-- 相关节点 -->
      <div class="mb-6 border-b border-gray-700/50 pb-4">
        <h4 class="text-blue-300 font-semibold mb-2">相关节点 ({{ relatedNodes.length }})</h4>
        <div class="space-y-2 text-sm">
          <div v-for="node in relatedNodes" :key="node.id" class="flex items-center justify-between">
            <div class="flex items-center">
              ...
              <span class="text-blue-200">{{ node.name }}</span>
            </div>
            <button 
              class="text-xs text-blue-300 hover:bg-blue-400/20 px-2 py-1 rounded"
      ...
      <!-- 节点操作 -->
      <div>
        <h4 class="text-blue-300 font-semibold mb-2">节点操作</h4>
        <div class="flex flex-col gap-2">
          <button 
            class="text-sm py-2 bg-blue-400/20 hover:bg-blue-400/30 text-blue-300 rounded-lg transition-colors flex items-center justify-center"
            @click="onFocusOnNode(selectedNode.id)"
          >
            <i class="fa-solid fa-search-plus mr-2"></i> 聚焦节点
          </button>
          <button 
            class="text-sm py-2 bg-gray-800 hover:bg-gray-700 text-blue-200 rounded-lg transition-colors flex items-center justify-center"
