<template>
  <div id="displayArea" class="flex-1 flex p-0 pl-0 pr-0 pt-0 mt-8 bg-light dark:bg-dark-primary h-[calc(100vh-2rem)] overflow-hidden" :class="{ 'transition-all duration-300': !isInitialLoading }">
    ...
    <div 
      ...
      :style="{
        width: settingsStore.leftNavVisible ? settingsStore.leftNavWidth : 'auto',
        minWidth: settingsStore.leftNavVisible ? '200px' : 'auto',
        maxWidth: settingsStore.leftNavVisible ? '370px' : 'auto',
        flexShrink: 0
      }"
    >
      <!-- 左侧面板header -->
      <div class="panel-header p-3 flex justify-between items-center transition-all duration-300">
        <!-- 历史面板：显示隐藏按钮和新增会话按钮 -->
        <div v-if="settingsStore.activePanel !== 'settings'" class="flex space-x-2">
          <!-- 隐藏左侧面板按钮 -->
          <ActionButton 
            icon="bars"
            title="隐藏左侧面板"
      ...
      <!-- 面板内容 - 使用复合组件简化逻辑 -->
      <div v-if="settingsStore.leftNavVisible" class="transition-all duration-300">
    ...
    <div 
      ...
      :style="{
        display: settingsStore.leftNavVisible ? 'block' : 'none'
      }"
    ...
    <div 
      ...
      :style="{
        width: settingsStore.rightPanelVisible ? settingsStore.rightPanelWidth : 'auto',
        minWidth: settingsStore.rightPanelVisible ? '200px' : 'auto',
        maxWidth: settingsStore.rightPanelVisible ? '370px' : 'auto',
        flexShrink: 0
      }"
    >
      <!-- 右侧面板header -->
