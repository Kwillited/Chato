          ...
          <div v-for="group in groupedChats" :key="group.title" class="mb-4 transition-all duration-300">
            ...
            <div class="space-y-1" v-if="!collapsedGroups[group.title]">
              <div
                ...
                :key="chat.id"
                class="p-2 rounded-lg cursor-pointer transition-all duration-300 ease-in-out hover:bg-gray-100 dark:hover:bg-gray-800 min-h-9 flex items-center relative focus-within:outline-2 focus-within:outline-gray-400 dark:focus-within:outline-gray-500 focus-within:outline-offset-2 border border-transparent"
                :class="{ 'font-semibold bg-gray-300 dark:bg-gray-700': isActiveChat(chat.id), pinned: chat.pinned }"
